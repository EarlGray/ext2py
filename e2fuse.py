...
class e2fuse(fuse.Fuse):
    ...
    def getattr(self, path):
        ...
        st.st_ino = ent.inode
        if self.conf['user']:
            (st.st_uid, st.st_gid) = (os.getuid(), os.getgid())
        else:
            (st.st_uid, st.st_gid) = (ino.uid, ino.gid)
        st.st_mode = ino.mode
    ...
    def statvfs(self):
        ...
        st.f_frsize = 0
        nfreeblocks = self.fs.sb.d['s_free_blocks_count']
        nrsrvblocks = self.fs.sb.d['s_r_blocks_count']
        st.f_bavail = nfreeblocks - nrsrvblocks
...
def main(argv):
    fsserv = e2fuse(version="%prog "+fuse.__version__,
